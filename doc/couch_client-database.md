...
To use PHP on Couch client, you have to create a PHPOnCouch\Client instance, setting the URL to your couchDB server, and the database name.

Example : connect to the couchDB server at http://my.server.com on port 5984 and on database mydb :

```php
$client = new PHPOnCouch\Client("http://my.server.com:5984/","mydb");
```
...
```php
$client = new PHPOnCouch\Client("http://couchAdmin:secret@my.server.com:5984/","mydb");
```

You can also tell PHPOnCouch\Client to use cookie based authentification, by passing an additional flag "cookie_auth" set to TRUE in the options array, as the third parameter of the PHPOnCouch\Client constructor.

Example : as the previous one, but using cookie based authentification

```php
$client = new PHPOnCouch\Client("http://couchAdmin:secret@my.server.com:5984/","mydb", array("cookie_auth"=>TRUE) );
```
...
```php
$client = new PHPOnCouch\Client("http://couch.server.com:5984/","hello");
...
Database names on CouchDB have restrictions : only lowercase characters (a-z), digits (0-9), and any of the characters _, $, (, ), +, -, and / are allowed. To test if a given database name is valid, use the static **isValidDatabaseName()** PHPOnCouch\Client method.

Example :

```php
$my_database = "user311(public)";
if ( PHPOnCouch\Client::isValidDatabaseName($my_database) ) {
    $client = new PHPOnCouch\Client("http://couch.server.com:5984/",$my_database);
} else {
...
```php
$client = new PHPOnCouch\Client("http://localhost:5984", "db1");
...
The callable function or method will receive two arguments : the JSON object decoded as a PHP object, and a PHPOnCouch\Client instance, allowing developers to issue CouchDB queries from inside the callback.
